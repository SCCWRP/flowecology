...
bsprdsdst <- reactive({
  ...
  # by watersheds
  p1 <- ggplot(bsprds, aes(x = shed, y = prd, fill = prd)) + 
  ...
  # all 
  p2 <- ggplot(bsprds, aes(x = 'All selected', y = prd, fill = prd)) + 
    geom_violin(draw_quantiles = 0.5, alpha = 0.8) + 
    labs(y = NULL) +
    scale_y_continuous(limits = c(0, 1)) +
    pdstthm
...
futprdsdst <- reactive({
  ...
  # by watersheds
  p1 <- ggplot(futprds, aes(x = shed, y = prd, fill = prd)) + 
    geom_violin(draw_quantiles = 0.5, alpha = 0.8) + 
    labs(y = "Pr. occurrence") +
    scale_y_continuous(limits = c(0, 1)) +
    pdstthm
  ...
  # all 
  p2 <- ggplot(futprds, aes(x = 'All selected', y = prd, fill = prd)) + 
    geom_violin(draw_quantiles = 0.5, alpha = 0.8) + 
    labs(y = NULL) +
    scale_y_continuous(limits = c(0, 1)) +
    pdstthm
...
plotOutput('bsprdsdst', height = 400) %>% withSpinner()
```
...
plotOutput('futprdsdst', height = 400) %>% withSpinner()
```